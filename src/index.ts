...
import { getLocalDb, flushPersist as persistLocalDb } from './services/local-db';
...
async function quitPhevere(): Promise<void> {
  ...
  try {
    await stopSelectionMonitoring();
  } catch (e) {
    log.warn('main', 'stopSelectionMonitoring on quit', { err: String(e) });
  }
  try {
  ...
  // Hard exit after cleanup — app.quit() alone can leave native addon / ORT children.
  setTimeout(() => {
    try {
      app.exit(0);
    } catch {
