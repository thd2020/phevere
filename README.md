...
- **Native text selection** — UIAutomation on Windows (best as Administrator), plus a silent Ctrl+C fallback after a drag/double-click when Chromium fires an empty UIA event (Cursor agent chat, some VS Code webviews). On macOS, Accessibility first (including Chromium text-markers), then Safari/Chrome AppleScript, then a silent Cmd+C if the app still exposes no selected text. Shortcut / hover / OCR modes in Settings. Select in the main window or an open lookup to get the same toolstrip as selecting elsewhere (the open lookup is left alone). A second selection (even before the first popup finishes loading) follows the latest text and position. After following a word-family or wiki link, **Back / Forward** on the dictionary tab (and the mouse extra buttons) restore the previous lookup.
