...
**2026-09-04:** Lexicon sidebar is two layers: outer card + vertical split, then one hairline per block. Section tabs pile inside a block (POS, family rows, synonyms/antonyms). A block with one section shows that name only. No bold rules in the senses.
