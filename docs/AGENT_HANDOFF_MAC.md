...
**2026-09-04:** Toolbar 🔊 is recorded audio; IPA chip speakers synthesize that transcription. Lexicon POS is a skinny vertical-label rail (no **all**; first POS is the top). Mac capture actions match Windows (read-window, Music/Spotify now-playing, Screen Recording prompt, inset traffic lights, ⌘ keycaps). Selection backend is no longer a draft.
