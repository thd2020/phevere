...
**2026-09-04:** Toolbar 🔊 is recorded audio; IPA chip speakers synthesize that transcription. Lexicon is an explorer pane (vertical rail includes examples/synonyms/family; nested 1px frames). Mac capture actions match Windows (read-window, Music/Spotify now-playing, Screen Recording prompt, inset traffic lights, ⌘ keycaps). Selection backend is no longer a draft.
